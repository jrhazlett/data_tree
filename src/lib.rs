...
mod test {
    ...
    impl ExampleTree {
        ...
        */
        ...
        pub fn get_path_from_test_data( arg_string_test_data: &str ) -> Vec<String> { arg_string_test_data[ ExampleTree::get_string_prefix_test_data().len().. ].split("_").map(|item|{item.to_string()}).collect() }
        //
        // Public - get - string
        //
        // This provides a hard-coded prefix for consistency across all functions that depend on it
        fn get_string_prefix_test_data() -> String { "test_data_".to_string() }
        ...
        pub fn get_string_test_data_via_path( arg_path: &[&str] ) -> String { [ ExampleTree::get_string_prefix_test_data(), arg_path.join( "_" ) ].join("") }
        //
        // Public - get - vec
        //
        //
    ...
    fn test_get_node_mute_at_key() {
        ...
        let node_mut =
            match node_root.get_node_mut_at_key( "a" ) {
                Some( result ) => result,
    ...
    fn test_get_path_to_node() {
        ...
        assert_eq!( path_to_node, path_expected.iter().map(|item|{item.to_string()}).collect::<Vec<String>>() )
    }
    ...
    fn test_get_vec_of_data_along_path() {
        ...
        let vec_result =
            match node_root.get_vec_of_nodes_along_path( &path ) {
                Some( result ) => result,
                None => panic!( "Failed to return vec of data with correct path." )
            }.iter().map(|item_node| {item_node.field_string_data.clone()}).collect::<Vec<String>>();

        assert_eq!( vec_result, [ "", "", data ].iter().map( |item| {item.to_string()} ).collect::<Vec<String>>() );
    }
    ...
    fn test_get_vec_of_nodes_satisfying_callback() {
        ...
        let vec_of_nodes = node_root.get_vec_of_nodes_satisfying_callback(
            |arg_node | { arg_node.field_string_data == string_test_data_expected.to_string() }
        );
    ...
    fn test_get_vec_of_pairs_paths_and_nodes_with_data_satisfying_callback_sorted() {
        ...
        let vec_of_pairs_paths_and_nodes = node_root.get_vec_of_pairs_paths_and_nodes_with_data_satisfying_callback_sorted(
            |arg_node | { arg_node.field_string_data == string_test_data_expected.to_string() }
        );
    ...
    fn test_get_vec_of_paths_to_nodes_satisfying_callback_sorted() {
        ...
        let vec_of_paths = node_root.get_vec_of_paths_to_nodes_satisfying_callback_sorted(
            |arg_node | { arg_node.field_string_data == string_test_data_expected.to_string() }
        );
    ...
    fn test_get_vec_of_nodes_along_path() {
        ...
        let vec_of_nodes_result =
            match node_root.get_vec_of_nodes_along_path( &path ) {
                Some( result ) => result,
                None => panic!( "Failed to get vec of nodes." )
            };

        let vec_result = vec_of_nodes_result.iter().map( |item_node| { item_node.field_string_data.as_str() } ).collect::<Vec<&str>>();
        let vec_expected = [ "", "", data ].iter().map(|item| {*item}).collect::<Vec<&str>>();
    ...
    fn test_get_vec_of_nodes_in_tree() {
        ...
        let vec_of_strings = node_root.get_vec_of_nodes_in_tree().iter().map(|item|{item.field_string_data.as_str()}).collect::<Vec<&str>>();
    ...
    fn test_get_vec_of_paths_in_tree() {
        ...
        let vec_expected = expected
            .iter()
            .map( |item| {
                ( *item ).iter().map( |item| { *item } ).collect::<Vec<&str>>()
            } ).collect::<Vec<Vec<&str>>>();
    ...
    fn test_insert_node_at_key() {
        ...
        let node_at_key =
            match node_root.get_node_at_key( key ) {
                Some( result ) => result,
    ...
    fn test_insert_node_at_path() {
        ...
        let node_at_path =
            match node_root.get_node_at_path_or_error( &path ) {
                Ok( result ) => result,
                Err( err ) => panic!( "{}", err )
            };
    ...
    fn test_pop_all_children_and_return_vec_of_pairs_keys_and_child_nodes() {
        ...
        let vec_of_pairs_result =
            match node_root.pop_all_children_and_return_vec_of_pairs_keys_and_child_nodes() {
                Some( result ) => result,
                None => panic!( "Failed: pop_all_children_and_return_vec_of_pairs_keys_and_child_nodes()" )
            };
        //
        // Validate keys
        //
        let mut vec_of_keys_result = vec_of_pairs_result.iter().map( |(item_key, _item_node)| { item_key.as_str() } ).collect::<Vec<&str>>();
        vec_of_keys_result.sort_by(|item_left, item_right|item_left.cmp(item_right));

        let vec_of_keys_expected = slice_of_keys.iter().map(|item| {*item}).collect::<Vec<&str>>();

        assert_eq!( vec_of_keys_result, vec_of_keys_expected, );

        let mut vec_of_test_data_result = vec_of_pairs_result.iter().map(|(_item_key, item_node)| {item_node.field_string_data.clone()}).collect::<Vec<String>>();
    ...
    fn test_pop_node_at_key() {
        ...
        node_child =
            match node_root.pop_node_at_key( key ) {
                Some( result ) => result,
                None => panic!( "Failed to pop node at key: {}", key )
            };

        if node_root.has_key( key ) { panic!( "Error: node_root still has key: {}", key ) }
    ...
    fn test_pop_node_at_path() {
        ...
        node_child =
            match node_root.pop_node_at_path( &path ) {
                Some( result ) => result,
                None => panic!( "Failed to pop node at path: {:?}", &path )
            };

        if node_root.has_path( &path ) { panic!( "Error: node_root still has path: {:?}", &path ) }
    ...
    fn test_pop_node_at_path_or_error() {
        ...
        assert_eq!( node_popped.field_string_data, ExampleTree::get_string_test_data_via_path( &path ) );
    }
    ...
    fn test_pop_node_at_key_and_promote_its_children() {
        ...
        assert_eq!( node_root.get_vec_of_keys_all_ending_with_suffix_sorted( "_promoted" ), expected_filtered )
    }
    ...
    fn test_pop_node_at_path_and_promote_its_children() {
        ...
        let node_root_child =
            match node_root.get_node_mut_at_path( &path ) {
                Some( result ) => result,
                None => panic!( "Failed to get child node." )
            };
        ...
        let node_parent =
            match node_root.get_node_at_path_or_error( &path_parent_of_popped_node ) {
                Ok( result ) => result,
                Err( err ) => panic!( "{}", err )
            };
    ...
    fn test_raise_error_if_path_is_invalid() {
        ...
        match node_root.raise_error_if_path_is_invalid( &path_invalid ) {
            Ok(()) => panic!( "raise_error_if_path_is_invalid() failed to raise error on invalid path." ),
            Err( _err ) => {}
        }
    }
}
...
impl Node {
    ...
    pub fn clear_children( &mut self ) { self.field_hash_map_children.clear() }
    //
    // Public - get - count
    //
    ...
    pub fn get_count_of_leaves( &self ) -> usize {
        ...
        let mut stack = self.field_hash_map_children.values().collect::<Vec<&Node>>();
        loop {
            let item_node = match stack.pop() { Some( result ) => result, None => break };
            if item_node.field_hash_map_children.is_empty() { int_count_to_return += 1;
            } else { stack.extend( item_node.field_hash_map_children.values() ) }
        }
        int_count_to_return
    }
    ...
    // No test needed, simple wrapper
    pub fn get_count_of_node_children( &self ) -> usize { self.field_hash_map_children.values().len() }
    //
    // Public - get - node
    //
    ...
    pub fn get_node_at_key( &self, arg_key: &str ) -> Option<&Node> { match self.field_hash_map_children.get( &arg_key.to_string() ) { Some( node_result ) => Some( node_result ), None => None } }
    ...
    pub fn get_node_at_path( &self, arg_path: &[&str] ) -> Option<&Node> {
        ...
        for item_key in arg_path { node_to_return = match node_to_return.field_hash_map_children.get( &item_key.to_string() ) { Some( node_result ) => node_result, None => return None } }
        Some( node_to_return )
    }
    ...
    pub fn get_node_at_path_or_error( &self, arg_path: &[&str] ) -> Result<&Node, String> {
        ...
        let mut node_to_return = self;
        for item_key in arg_path { match node_to_return.field_hash_map_children.get( &item_key.to_string() ) { Some( node_result ) => { node_to_return = node_result; vec_path_that_exists.push( *item_key ); }, None => { return Err(Node::raise_error_because_path_failed( item_key, &arg_path, &vec_path_that_exists, )); } } }
        Ok( node_to_return )
    }
    //
    // Public - get - node mut
    ...
    pub fn get_node_mut_at_key( &mut self, arg_key: &str ) -> Option< &mut Node > { match self.field_hash_map_children.get_mut( &arg_key.to_string() ) { Some( node_result ) => Some( node_result ), None => None } }
    ...
    pub fn get_node_mut_at_path( &mut self, arg_path: &[&str] ) -> Option< &mut Node > {
        let mut node_to_return = self;
        for item_key in arg_path { node_to_return = match node_to_return.field_hash_map_children.get_mut( &item_key.to_string() ) { Some( node_result ) => node_result, None => return None } }
        Some( node_to_return )
    }
    ...
    pub fn get_node_mut_at_path_or_error( &mut self, arg_path: &[&str] ) -> Result< &mut Node, String > {
        match arg_path.len() {
            ...
            _ => {
                let mut vec_path_that_exists = Vec::new();
                let mut node_to_return = self;
                for item_key in arg_path { match node_to_return.field_hash_map_children.get_mut( &item_key.to_string() ) { Some( node_result ) => { vec_path_that_exists.push( *item_key ); node_to_return = node_result }, None => { return Err(Node::raise_error_because_path_failed(item_key, arg_path, &vec_path_that_exists, )); } } }
                Ok( node_to_return )
            }
        }
    }
    ...
    pub fn get_node_parent( &self, arg_node: &Node ) -> Option<&Node> {
        if eq( arg_node, self ) { return None }
        let mut stack = self.field_hash_map_children.values().map(|item| (self, item)).collect::<Vec<(&Node, &Node)>>();
        loop {
            let ( item_node_parent, item_node ) = match stack.pop() {
                Some( result ) => result,
                None => return None
            };
            if eq( item_node, arg_node ) { return Some( item_node_parent ) }
            stack.extend( item_node.field_hash_map_children.values().map( |item_node_sub| {(item_node, item_node_sub)} ).collect::<Vec<(&Node, &Node)>>() );
        }
    }
    //
    // Public - get - path
    ...
    pub fn get_key_to_node( &self, arg_node: &Node ) -> Option<String> {
        for ( item_key, item_node ) in &self.field_hash_map_children {
            if eq( item_node, arg_node ) { return Some( item_key.clone() ) }
        }
        None
    }
    ...
    pub fn get_path_to_node( &self, arg_node: &Node ) -> Option<Vec<String>> {
        let mut stack = self.field_hash_map_children.iter().map(|(item_key, item_node)| {(vec![item_key.as_str()], item_node)}).collect::<Vec<(Vec<&str>, &Node)>>();
        loop {
            let ( item_path, item_node ) = match stack.pop() { Some( ( path, node_current ) ) => ( path, node_current ), None => return None };
            if eq( item_node, arg_node ) { return Some( item_path.iter().map(|item|{item.to_string()}).collect() ) }
            //
            // Prep next iteration
            //
    ...
    pub fn get_vec_of_keys( &self ) -> Vec<String> { self.field_hash_map_children.keys().map(|item|{item.clone()}).collect() }
    ...
    pub fn get_vec_of_keys_all_ending_with_suffix( &self, arg_suffix: &str ) -> Vec<String> { self.field_hash_map_children.keys().filter( |item| { item.ends_with( arg_suffix ) } ).map(|item| {item.clone()}).collect::<Vec<String>>() }
    ...
    pub fn get_vec_of_keys_all_ending_with_suffix_sorted( &self, arg_suffix: &str ) -> Vec< String > {
        let mut vec_to_return = self.field_hash_map_children.keys().filter( |item| { item.ends_with( arg_suffix ) } ).map(|item| {item.clone()}).collect::<Vec<String>>();
        vec_to_return.sort_by( |item_left, item_right | { item_left.cmp( item_right ) } );
        vec_to_return
    }
    //
    ...
    pub fn get_vec_of_node_children( &self ) -> Vec<&Node> { self.field_hash_map_children.values().collect::<Vec<&Node>>() }
    ...
    pub fn get_vec_of_node_leaves( &self ) -> Vec<&Node> {
        ...
        loop {
            let item_node = match stack.pop() { Some( result ) => result, None => break };
            if item_node.field_hash_map_children.is_empty() { vec_to_return.push( item_node )
            } else { stack.extend( item_node.field_hash_map_children.values() ) }
        }
        vec_to_return
    }
    ...
    pub fn get_vec_of_nodes_along_path( &self, arg_path: &[&str] ) -> Option< Vec<&Node> > {
        ...
        for item_key in arg_path {
            ...
            item_node_current = match item_node_current.field_hash_map_children.get( &item_key.to_string() ) { Some( result ) => result, None => return None };
    ...
    pub fn get_vec_of_nodes_satisfying_callback( &self, arg_callback: impl Fn(&Node) -> bool ) -> Vec<&Node> {
        ...
        loop {
            let item_node = match stack.pop() { Some( result ) => result, None => break };
            if arg_callback( item_node ) { vec_to_return.push( item_node ) }
            stack.extend( item_node.field_hash_map_children.values() );
        }
        vec_to_return
    }
    ...
    pub fn get_vec_of_pairs_keys_and_node_children( &self ) -> Vec< ( String, &Node ) > { self.field_hash_map_children.iter().map(|(item_string, item_node)| { (item_string.clone(), item_node) }).collect() }

    /// Returns a vec of tuple-pairs referring to the keys and node children
    // No test needed; simple collection
    pub fn get_vec_of_pairs_keys_and_node_mut_children( &mut self ) -> Vec< ( String, &mut Node ) > { self.field_hash_map_children.iter_mut().map(|(item_string, item_node)| {(item_string.clone(), item_node)}).collect() }
    ...
    pub fn get_vec_of_pairs_paths_and_nodes_with_data_satisfying_callback( &self, arg_callback: impl Fn(&Node) -> bool ) -> Vec<(Vec<String>, &Node)> {
        ...
        loop {
            ...
            // Pop from stack
            //
            let ( item_path, item_node ) = match stack.pop() { Some( result ) => result, None => break };
            //
            // Do comparison and add to return value
            //
            if arg_callback( item_node ) { vec_to_return.push( ( item_path.to_vec(), item_node ) ) }
            //
            // Prep next iteration
            //
            for ( item_key_sub, item_node_sub ) in &item_node.field_hash_map_children {
        ...
        vec_to_return.iter().map(|(item_path, item_node)|{ (item_path.iter().map(|item|{item.to_string()}).collect::<Vec<String>>(), *item_node) }).collect()
    }
    ...
    /// * arg_callback: Receives a node and returns bool
    // Test: test_get_vec_of_pairs_paths_and_nodes_with_data_satisfying_callback_sorted()
    pub fn get_vec_of_pairs_paths_and_nodes_with_data_satisfying_callback_sorted( &self, arg_callback: impl Fn(&Node) -> bool ) -> Vec<(Vec<String>, &Node)> { Node::get_vec_of_pairs_paths_and_generics_sorted( &self.get_vec_of_pairs_paths_and_nodes_with_data_satisfying_callback( arg_callback ) ) }
    //
    ...
    pub fn get_vec_of_paths_in_tree( &self ) -> Vec<Vec<String>> {
        ...
        vec_to_return.iter().map(|item|{item.iter().map(|item|{item.to_string()}).collect::<Vec<String>>()}).collect()
    }

    /// Returns a vec of all paths within the tree, sorted
    // Test: test_get_vec_of_paths_in_tree()
    pub fn get_vec_of_paths_in_tree_sorted( &self ) -> Vec<Vec<String>> { Node::get_vec_of_paths_sorted(&self.get_vec_of_paths_in_tree()) }
    ...
    pub fn get_vec_of_paths_to_node_leaves( &self ) -> Vec<Vec<String>> {
        ...
        loop {
            let ( item_path, item_node ) = match stack.pop() { Some( result ) => result, None => break };
            if item_node.field_hash_map_children.is_empty() { vec_to_return.push( item_path.to_vec() )
            } else {
                for ( item_key_sub, item_node_sub ) in &item_node.field_hash_map_children {
                    stack.push( (
                        {
    ...
    pub fn get_vec_of_paths_to_nodes_satisfying_callback( &self, arg_callback: impl Fn(&Node) -> bool ) -> Vec<Vec<String>> {
        ...
        loop {
            let ( item_path, item_node ) = match stack.pop() { Some( result ) => result, None => break };
            if arg_callback( item_node ) { vec_to_return.push( item_path.to_vec() ) }
            for ( item_key_sub, item_node_sub ) in &item_node.field_hash_map_children {
                stack.push( (
                    {
                        let mut item_path_sub = item_path.to_vec();
        ...
        vec_to_return.iter().map(|item| {item.iter().map(|item|{item.to_string()}).collect::<Vec<String>>()}).collect::<Vec<Vec<String>>>()
    }
    ...
    pub fn get_vec_of_paths_to_nodes_satisfying_callback_sorted( &self, arg_callback: impl Fn(&Node) -> bool ) -> Vec<Vec<String>> {
        ...
        vec_to_return.sort_by(|item_path_left, item_path_right| { item_path_left.join(".").cmp(&item_path_right.join(".")) });
        vec_to_return
    }
    //
    // Public - insert
    ...
    pub fn insert_node_at_key( &mut self, arg_key: &str, arg_node: Node ) -> Result<(), String> {
        match Node::raise_error_if_key_is_invalid( arg_key ) { Ok(()) => {}, Err( err ) => return Err( err ) }
        self.field_hash_map_children.insert( arg_key.to_string(), arg_node );
        Ok(())
    }
    ...
    pub fn insert_node_at_path( &mut self, arg_path: &[&str], arg_node: Node ) -> Result< (), String > {
        match arg_path.len() {
            ...
            _ => {
                ...
                match self.raise_error_if_path_is_invalid( &arg_path ) { Ok(()) => {}, Err(err) => return Err(err) };
                ...
                for item_key in &arg_path[ 0..arg_path.len() - 1 ] {
                    ...
                    if node_current.field_hash_map_children.contains_key( &item_key.to_string() ) { node_current = match node_current.field_hash_map_children.get_mut( &item_key.to_string() ) { Some( result ) => result, None => return Err( "Error: Failed to get node child that's definitely there.".to_string() ) }
                    } else {
                        node_current.field_hash_map_children.insert( item_key.to_string(), Node::new() );
                        node_current = match node_current.field_hash_map_children.get_mut( &item_key.to_string() ) { Some( result ) => result, None => return Err( "Error: Failed to get node child that's definitely there.".to_string() ) }
                    }
                }
                node_current.field_hash_map_children.insert( arg_path[ arg_path.len() - 1 ].to_string(), arg_node );
            }
        }
        Ok( () )
    }
    ...
    pub fn has_children( &self ) -> bool { self.field_hash_map_children.len() > 0 }

    /// Returns true if the string within the node contains any data
    // No test needed since this is a wrapper
    pub fn has_data( &self ) -> bool { self.field_string_data.len() > 0 }

    /// Returns true if the key exists for an immediate child
    /// # Arguments
    /// * arg_key: A key of type &str used to identify a child node
    // No test needed since this is a wrapper
    pub fn has_key( &self, arg_key: &str ) -> bool { self.field_hash_map_children.contains_key( &arg_key.to_string() ) }
    ...
    pub fn has_path( &self, arg_path: &[&str] ) -> bool {
        let mut node_current = self;
        for item_key in arg_path { node_current = match node_current.field_hash_map_children.get( &item_key.to_string() ) { Some( result ) => result, None => return false } }
        true
    }
    ...
    pub fn is_path_valid( &self, arg_path: &[&str] ) -> bool {
        if arg_path.is_empty() { return false }
        for item_key in arg_path {
            if item_key.is_empty() { return false }
        }
        true
    }
    //
    ...
    pub fn pop_all_children_and_return_vec_of_pairs_keys_and_child_nodes( &mut self ) -> Option< Vec<(String, Node)> > {
        ...
        for item_key in self.field_hash_map_children.keys().map(|item|{item.clone()}).collect::<Vec<String>>() { vec_to_return.push( ( item_key.clone(), match self.field_hash_map_children.remove( &item_key ) { Some( result ) => result, None => return None } ) ); }
        Some( vec_to_return )
    }
    ...
    pub fn pop_node_at_key_and_promote_its_children( &mut self, arg_key: &str, arg_collision_suffix: &str ) -> Result< Node, String > {
        let node_to_pop = match self.field_hash_map_children.get( arg_key ) { Some( result ) => result, None => { return Err([ "Error: arg_key is not a child.".to_string(), format!( "arg_key = {}", arg_key ) ].join("\n")) } };
        //
        ...
        for item_key in node_to_pop.field_hash_map_children.keys() {
            ...
            vec_of_pairs_keys_in_node_to_pop_and_keys_with_suffixes.push( ( item_key.clone(), item_key_with_suffix.clone() ) );
            if self.field_hash_map_children.contains_key( &item_key_with_suffix ) { vec_of_collisions_detected.push( item_key_with_suffix.clone() ); }
        }
        if vec_of_collisions_detected.len() > 0 { return Err( [ "Error: Collisions detected with suffix.".to_string(), format!( "vec_of_collisions_detected = {:?}", vec_of_collisions_detected ), ].join( "\n" ) ) }
        //
        // If we get this far, then there are no collisions
        //
        let mut node_popped = match self.field_hash_map_children.remove( arg_key ) { Some( result ) => result, None => return Err([ "Error: Failed key doesn't exist.".to_string(), format!( "arg_key = {}", arg_key ), ].join("\n")) };
        for ( item_key_to_pop, item_key_with_suffix ) in vec_of_pairs_keys_in_node_to_pop_and_keys_with_suffixes { self.field_hash_map_children.insert( item_key_with_suffix.to_string(), match node_popped.field_hash_map_children.remove( item_key_to_pop.as_str() ) { Some( result ) => result, None => { return Err([ "Error: Failed to pop child from node_popped.".to_string(), format!( "item_key_to_pop = {}", item_key_to_pop, ), format!( "slice of valid keys = {:?}", self.field_hash_map_children.keys() ) ].join("\n")) } } ); }
        Ok( node_popped )
    }
    ...
    pub fn pop_node_at_path_and_promote_its_children( &mut self, arg_path: &[&str], arg_collision_suffix: &str ) -> Result< Node, String > {
        match arg_path.len() {
            1 => return self.pop_node_at_key_and_promote_its_children( arg_path[ 0 ], arg_collision_suffix ),
            _ => {
                match self.raise_error_if_path_is_invalid( &arg_path ) { Ok(()) => {}, Err( err ) => return Err( err ) }
                let index_last = arg_path.len() - 1;
                let node_parent = match self.get_node_mut_at_path_or_error( &arg_path[ 0..index_last ] ) { Ok( result ) => result, Err( err ) => return Err( err ) };
                match node_parent.pop_node_at_key_and_promote_its_children( arg_path[ index_last ], arg_collision_suffix ) { Ok( result ) => return Ok( result ), Err( err ) => return Err( err ) }
            }
        }
    }
    ...
    pub fn pop_node_at_key( &mut self, arg_key: &str ) -> Option< Node > { match self.field_hash_map_children.remove( &arg_key.to_string() ) { Some( result ) => Some( result ), None => None } }
    ...
    pub fn pop_node_at_path( &mut self, arg_path: &[&str] ) -> Option< Node > {
        match arg_path.len() {
            ...
            _ => {
                let mut node_current = self;
                for item_key in &arg_path[ 0..arg_path.len() - 1 ] { node_current = match node_current.field_hash_map_children.get_mut( &item_key.to_string() ) { Some( result ) => result, None => return None }; }
                match node_current.field_hash_map_children.remove( &arg_path[ arg_path.len() - 1 ].to_string() ) { Some( result ) => Some( result ), None => None }
            }
        }
    }
    ...
    pub fn pop_node_at_path_or_error( &mut self, arg_path: &[&str] ) -> Result< Node, String > {
        match arg_path.len() {
            0 => return Err( "Error: arg_path is empty.".to_string() ),
            1 => { match self.pop_node_at_key( arg_path[ 0 ] ) { Some( result ) => return Ok( result ), None => { return Err( [ "Error: arg_path not found.".to_string(), format!( "arg_path = {:?}", arg_path ), ].join( "\n" ) ) } } }
            _ => {
                let mut vec_path_existing = Vec::new();
                let mut node_current = self;
                for item_key in &arg_path[ 0..arg_path.len() - 1 ] { node_current = match node_current.field_hash_map_children.get_mut( &item_key.to_string() ) { Some( result ) => { vec_path_existing.push( *item_key ); result }, None => { return Err(Node::raise_error_because_path_failed( item_key, arg_path, &vec_path_existing, )) } }; }
                match node_current.field_hash_map_children.remove( arg_path[ arg_path.len() - 1 ] ) { Some( result ) => Ok( result ), None => { return Err([ "Error: Failed to remove node at path.".to_string(), format!( "arg_path = {:?}", arg_path, ), format!( "part of path that exists = {:?}", vec_path_existing, ), ].join("\n")) } }
            }
        }
    }
    //
    ...
    fn raise_error_because_path_failed( arg_key_at_failure: &str, arg_path: &[&str], arg_vec_path_that_exists: &Vec<&str> ) -> String {
        return [
    ...
    pub fn raise_error_if_key_is_invalid( arg_key: &str ) -> Result<(), String> {
        if arg_key.is_empty() { return Err( "Error: arg_key is empty.".to_string() ) }
        Ok(())
    }
    ...
    pub fn raise_error_if_path_is_invalid( &self, arg_path: &[&str] ) -> Result< (), String > {
        if arg_path.is_empty() { return Err( [ "Error: arg_path is empty.".to_string() ].join("\n") ) }
        for item_key in arg_path {
            if item_key.is_empty() { return Err( [ "Error: Empty items detected in arg_path.".to_string(), format!( "arg_path = {:?}", arg_path ), ].join( "\n" ) ) }
        }
        Ok(())
    }
    //
    ...
    //
    fn get_vec_of_pairs_paths_and_generics_sorted<'l, T>(arg_vec_of_pairs: &Vec<(Vec<String>, &'l T)>) -> Vec<(Vec<String>, &'l T)> {
        let mut vec_to_sort = Vec::new();
        for ( item_path, item_t ) in arg_vec_of_pairs { vec_to_sort.push( ( item_path.join("\n"), item_path, item_t ) ); }
        vec_to_sort.sort_by( |(item_string_left, _item_vec_left, _item_t_left), (item_string_right, _item_vec_right, _item_t_right)| { item_string_left.cmp(item_string_right) } );
        let mut vec_to_return = Vec::new();
        for ( _item_string, item_path, item_t ) in vec_to_sort { vec_to_return.push(( item_path.to_vec(), *item_t, )) }
        vec_to_return
    }

    fn get_vec_of_paths_sorted( arg_vec_of_paths: &Vec<Vec<String>> ) -> Vec<Vec<String>> {
        let mut vec_to_sort = Vec::new();
        for item_path in arg_vec_of_paths { vec_to_sort.push((item_path.join("."), item_path)); }
        vec_to_sort.sort_by( |(item_string_left, _item_vec_left), (item_string_right, _item_vec_right)| { item_string_left.cmp(item_string_right) } );
        let mut vec_to_return = Vec::new();
        for (_item_string, item_path) in vec_to_sort { vec_to_return.push(item_path.to_vec()); }
        vec_to_return
    }
    //
    // Setup
